...
! read the input, define the mesh and obstacle paramters and intialize
           call read_input(nzm,nz,z,dz,dt,time_max,ntime,prtime,utau_x,utau_y,dpdx,dpdy,&
                           pb,ss,wx,wy,bx,by,hmean,vl,sf,lpz)
...
! compute the drag coefficent and the length scales
          ...
          call drag_length(nzm,nz,z,bx,by,wx,wy,hmean,lambdap,ceps,&
...
!-------------------------------------------------------------
           subroutine read_input(nzm,nz,z,dz,dt,time_max,ntime,prtime,utau_x,utau_y,dpdx,dpdy,pb,ss,wx,wy,bx,by,hmean,vl,sf,lpz)          
           implicit none
           integer nzm
! flow variables
...
!-----------------------------------------------------------------
      subroutine drag_length(nzm,nz,z,bx,by,wx,wy,hmean,lambdap,ceps,ck,cmu,cdragx,cdragy,dls,dlk,lpz)
      implicit none
! input
      integer nzm,nz,iz
      real z(nzm+1)
